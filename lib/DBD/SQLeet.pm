...
DBD::SQLeet - Self-contained RDBMS in a DBI Driver
...
B<DBD::SQLeet> is a Perl DBI driver for SQLite, that includes
...
Yes, B<DBD::SQLeet> is small and light, but it supports full transactions!
...
DBD::SQLeet is usually compiled with a bundled SQLite library
...
You can look at C<$DBD::SQLeet::sqlite_version> (C<3.x.y> format) or
C<$DBD::SQLeet::sqlite_version_number> (C<3xxxyyy> format)
to find which version of SQLite is actually used. You can also
check C<DBD::SQLeet::Constants::SQLITE_VERSION_NUMBER()>.

You can also find how the library is compiled by calling
C<DBD::SQLeet::compile_options()> (see below).
...
as well for finer control:
  ...
  my $dbh = DBI->connect("dbi:SQLeet:uri=file:$path_to_dbfile?mode=rwc");
...
You can set sqlite_open_flags (only) when you connect to a database:
  ...
  use DBD::SQLeet::Constants qw/:file_open/;
  my $dbh = DBI->connect("dbi:SQLeet:$dbfile", undef, undef, {
...
=head2 DBD::SQLeet And File::Temp
...
filename for DBD::SQLeet, but as noted in L<File::Temp>'s POD,
...
store natively as a BLOB use the following code:
  ...
  my $dbh = DBI->connect("dbi:SQLeet:dbfile","","");
...
This is because DBD::SQLeet assumes that all the bind values are text
...
to let DBD::SQLeet to see if the bind values are numbers or not.
...
You can set it to true when you connect to a database.
  ...
  my $dbh = DBI->connect('dbi:SQLeet:foo', undef, undef, {
    AutoCommit => 1,
    RaiseError => 1,
...
existing data in your databases created by DBD::SQLeet have not
...
Note that SQLite 3.7.15 (bundled with DBD::SQLeet 1.38_02) enhanced
...
(released on Oct 14, 2009; bundled in DBD::SQLeet 1.26_05).
...
DBI/DBD::SQLeet's transactions may be a bit confusing. They behave
...
cause a "deadlock". To avoid this, DBD::SQLeet internally issues
...
L<DBD::SQLeet> only processes the first statement, and discards the
rest.
...
Returns the version of the SQLite library which B<DBD::SQLeet> is using,
e.g., "2.8.0". Can only be read.

=item sqlite_unicode

If set to a true value, B<DBD::SQLeet> will turn the UTF-8 flag on for all
...
If you set this to true, DBD::SQLeet tries to issue a C<begin
...
If you set this to true, DBD::SQLeet tries to see if the bind values
are number or not, and does not quote if they are numbers. See above
for details.

=item sqlite_extended_result_codes

If set to true, DBD::SQLeet uses extended result codes where appropriate
...
without any implementation. The C<DBD::SQLeet> driver
...
An initial callback is already registered by C<DBD::SQLeet>,
so for most common cases it will be simpler to just
add your collation sequences in the C<%DBD::SQLeet::COLLATION>
...
is an integer equal to either C<DBD::SQLeet::INSERT>,
C<DBD::SQLeet::DELETE> or C<DBD::SQLeet::UPDATE>
(see L</"Action Codes">);
...
authorizer callback should return C<DBD::SQLeet::OK> to allow the
action, C<DBD::SQLeet::IGNORE> to disallow the specific action but
allow the SQL statement to continue to be compiled, or
C<DBD::SQLeet::DENY> to cause the entire SQL statement to be rejected
...
Virtual tables are explained in L<DBD::SQLeet::VirtualTable>.

=head1 DRIVER FUNCTIONS

=head2 DBD::SQLeet::compile_options()

Returns an array of compile options (available since SQLite 3.6.23,
bundled in DBD::SQLeet 1.30_01), or an empty array if the bundled
library is old or compiled with SQLITE_OMIT_COMPILEOPTION_DIAGS.

=head2 DBD::SQLeet::sqlite_status()
...
=head2 DBD::SQLeet::strlike($pattern, $string, $escape_char), DBD::SQLeet::strglob($pattern, $string)
...
the C<DBD::SQLeet> module must be explicitly C<use>d at compile
...
DELETE operation would be written as follows :
  ...
  $dbh->sqlite_set_authorizer(sub {
    ...
    return $action_code == DBD::SQLeet::DELETE ? DBD::SQLeet::DENY
                                               : DBD::SQLeet::OK;
  });

The list of constants implemented in C<DBD::SQLeet> is given
...
In addition, C<DBD::SQLeet> automatically installs the
...
created, C<DBD::SQLeet> offers a simpler interface through the
C<%DBD::SQLeet::COLLATION> hash : just insert your own
...
characters :
  ...
  use DBD::SQLeet;
  $DBD::SQLeet::COLLATION{no_accents} = sub {
    my ( $a, $b ) = map lc, @_;
    tr[����������������������������]
...
always grab the tied object underneath C<%DBD::SQLeet::COLLATION> ---
...
see L<DBD::SQLeet::Fulltext_search>.
...
C<DBD::SQLeet> also supports virtual tables implemented in I<Perl code>:
see L<DBD::SQLeet::VirtualTable> for using or implementing such
...
L<DBD::SQLeet::VirtualTable::FileContent> : implements a virtual
column that exposes file contents. This is especially useful
in conjunction with a fulltext index; see L<DBD::SQLeet::Fulltext_search>.

=item *

L<DBD::SQLeet::VirtualTable::PerlData> : binds to a Perl array
...
=head1 FOR DBD::SQLeet EXTENSION AUTHORS
...
header like this:
  ...
  my $sqlite3_h = catfile(dist_dir('DBD-SQLeet'), 'sqlite3.h');

  # or only a particular header, amalgamated in sqlite3.c
  my $what_i_want = 'parse.h';
  my $sqlite3_c = catfile(dist_dir('DBD-SQLeet'), 'sqlite3.c');
...
and you may want to add DBD::SQLeet to your extension's C<CONFIGURE_REQUIRES>
to ensure your extension users use the same C source/header they use
to build DBD::SQLeet itself (instead of the ones installed in their
system).
...
L<https://github.com/ddmitov/DBD-SQLeet>.
